...
**Project context (CryptoTerminal):**
- Angular 20 **zoneless** + **signals** only.
- Standalone components (no NgModules).
- Control flow: `@if`, `@for`, `@switch` only.
- Inputs: `input()`/`input.required()`; outputs: `output()`; two-way: `model()`.
- **Reactive Forms** for any user input (never template-driven or `[(ngModel)]`).
- DI via `inject()` (no constructor injection legacy).
- Architecture: `core/`, `features/`, `shared/` (no forbidden dependencies).
...
- Migrating legacy Angular patterns to zoneless + signals
- Simplifying complex templates or services in features
...
**Angular-specific guardrails (CryptoTerminal):**
- Never introduce `zone.js`, `ChangeDetectorRef`, or `*ngIf/*ngFor/*ngSwitch`.
- Avoid `effect()` unless strictly necessary; prefer `computed()` and pure helpers.
- Keep components under 200 lines; extract subcomponents or move logic to services/utils.
- Add `track` in `@for` loops.
- No function calls in templates; use `computed()` or precomputed signals.
...
- When it would break architectural boundaries (`core` ↔ `features` ↔ `shared`)
```
...
- [ ] Angular boundaries respected (core/features/shared)
...
- [ ] All tests pass
- [ ] Component tests use standalone TestBed + Vitest
